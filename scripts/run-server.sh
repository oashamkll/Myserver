...
# Restore autorun list
autorun run 2>/dev/null || true

# Start web server in SCREEN — survives browser close
sudo screen -dmS termux-server bash -c 'cd /root && python3 /root/server.py > /tmp/server.log 2>&1'
sleep 5
curl -sf http://localhost:8080 > /dev/null || { echo "Server failed!"; cat /tmp/server.log; exit 1; }
echo "[OK] Server running"

# Start ngrok in SCREEN
screen -dmS termux-ngrok bash -c 'ngrok http 8080 --log=stdout > /tmp/ngrok.log 2>&1'
sleep 12
...
[ -z "$PUBLIC_URL" ] && { echo "No ngrok URL!"; cat /tmp/ngrok.log; exit 1; }
...
git add SERVER_URL.txt && git commit -m "Live: $PUBLIC_URL" || true
git push origin HEAD:main || true

# Main loop: save every 10min, auto-restart after 5h30min
ELAPSED=0; MAX=19800; INTERVAL=600
echo "[*] Loop started: save every 10min, restart at 5h30min"
...
while [ $ELAPSED -lt $MAX ]; do
  ...
  # Health check ngrok
  if ! curl -s http://localhost:4040/api/tunnels | python3 -c \
    "import sys,json; d=json.load(sys.stdin); assert d.get('tunnels')" 2>/dev/null; then
    echo "[!] Ngrok dead, restarting..."
    screen -S termux-ngrok -X quit 2>/dev/null || true; sleep 2
...
")
    if [ -n "$NEW" ]; then
      ...
      git add SERVER_URL.txt && git commit -m "New URL: $NEW" || true
      git push origin HEAD:main || true
      echo "[OK] New URL: $NEW"
    fi
  fi
...
echo "[*] 5h30min elapsed — saving and restarting..."
save-state

# Trigger next run via push to main
MAIN_SHA=$(git rev-parse HEAD)
BASE_TREE=$(curl -s -H "Authorization: token ${GH_TOKEN}" \
...
echo "[OK] Next run triggered via commit!"
sleep 300
