...
for FILE in workspace.tar.gz.b64 pip_packages.txt apt_packages.txt; do
  ...
  [ "$HTTP" != "200" ] && echo "[*] No $FILE yet" && continue
  ...
  case "$FILE" in
    *.tar.gz.b64)
      ...
      ;;
    pip_packages.txt)
      sudo pip install -r "/tmp/${FILE}" -q 2>/dev/null && echo "[OK] Pip restored"
      ;;
    apt_packages.txt)
      xargs -a "/tmp/${FILE}" sudo apt-get install -y -qq 2>/dev/null && echo "[OK] Apt restored"
      ;;
  esac
done
echo "[*] Restore done"
