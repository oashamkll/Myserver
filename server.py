...
.bold{font-weight:bold} .dim{opacity:.5} .it{font-style:italic}
.bg1{background:#f85149} .bg2{background:#3fb950} .bg4{background:#58a6ff}
...
.sbtn {
  background: var(--blue); border: none; color: #000; width: 36px; height: 36px;
...
<div class="qkbar">
  ...
  <span class="qk" onclick="run('cd ~')">~</span>
  ...
  <span class="qk" onclick="type('sudo ')">sudo</span>
  <span class="qk" onclick="type('nano ')">nano</span>
  <span class="qk" onclick="type('cat ')">cat</span>
  <span class="qk" onclick="type('grep ')">grep</span>
  <span class="qk" onclick="type('chmod +x ')">chmod</span>
  <span class="qk" onclick="type('apt install -y ')">apt</span>
  <span class="qk" onclick="type('| ')">pipe</span>
...
<div class="overlay" id="ov">
  <div class="card">
    <h3>🔌 Disconnected</h3>
    <p>Connection to server lost.<br/>Click to reconnect.</p>
...
function ansi(str) {
  ...
  let html = '', bold=false, dim=false, it=false, fg=-1, bg=-1;

  const cls = () => (bold||dim||it||fg>=0||bg>=0) ? '</span>' : '';
  const opn = () => {
    const c=[];
    if(bold) c.push('bold'); if(dim) c.push('dim'); if(it) c.push('it');
    if(fg>=0) c.push('f'+fg); if(bg>=0) c.push('bg'+bg);
...
// ─── WRITE ───────────────────────────────────────
function write(text){
  out.innerHTML += ansi(text);
  tw.scrollTop = tw.scrollHeight;
}
function writeSys(msg, col){
  out.innerHTML += '<span style="color:'+col+';font-style:italic">'+
    msg.replace(/</g,'&lt;')+'</span>\\n';
  tw.scrollTop = tw.scrollHeight;
}

// ─── STATUS ──────────────────────────────────────
function setOk(msg){ dot.className='dot'; stxt.textContent=msg; alive=true; }
function setOff(msg){ dot.className='dot off'; stxt.textContent=msg; alive=false; }
...
function connect(){
  ...
  ws.onopen = ()=>{
    setOk('Ubuntu · Connected');
    writeSys('\\n✅  Connected to MyServer — Ubuntu Linux', '#3fb950');
    writeSys('💡  Type commands below or tap quick keys\\n', '#8b949e');
    // send terminal size
    sendPkt({type:'resize', cols: Math.floor(tw.clientWidth/7.2)|0 || 100, rows:40});
  };
...
const CTRL={
  ...
  tab:'\x09', a:'\x01', e:'\x05', u:'\x15', k:'\x0b'
};
function doCtrl(k){ sendRaw(CTRL[k]||k); inp.focus(); }
...
function onKey(e){
  if(e.key==='Enter'){ e.preventDefault(); send(); }
    ...
    if(CTRL[k]){ e.preventDefault(); doCtrl(k); }
  }
}
...
async function uploadFile(input){
  ...
  try{
    ...
    if(d.ok){ writeSys('✅ Saved to: '+d.path, '#3fb950'); run('ls -lh "'+d.path+'"'); }
    else writeSys('❌ Error: '+d.error, '#f85149');
...
async def handle_ws(request):
    ...
    # Spawn real bash via PTY — loop so logout/exit auto-restarts shell
    while True:
        pid, fd = pty.fork()
        ...
        if pid == 0:
            ...
            env['PS1'] = r'\[\033[01;32m\]\u@myserver\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]\$ '
            ...
            if ws.closed:
                break  # client disconnected — stop loop

            # Shell exited but client still connected — restart bash
